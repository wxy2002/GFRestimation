...
class mlp(nn.Module):
    def __init__(self, n_channels, n_classes):
        super().__init__()
        self.model_a = UNet3D(n_channels=n_channels, n_classes=n_classes)
        self.model_v = UNet3D(n_channels=n_channels, n_classes=n_classes)
        # 读取预训练模型
        print("Loading pretrained models...")
        self.model_a.load_state_dict(torch.load('model/Model2/best_model_arterial.pth'))
        self.model_v.load_state_dict(torch.load('model/Model2/best_model_venous.pth'))
    ...
    def forward(self, a, v, clin, pre_1=None, pre_2=None):
        ...
        output_l = output_l[:, 0] * out_a_l + output_l[:, 1] * out_v_l + output_l[:, 2] * pre_1.reshape(-1)
        ...
        return torch.cat([output_l, output_r], dim=1)
