...
class SEModule(nn.Module):
    ...
    def __init__(self, channel, reduction=16):
        ...
        self.se = nn.Sequential(
            nn.Conv3d(channel, reduction_channel, 1, bias=False),
            nn.ReLU(),
            nn.Conv3d(reduction_channel, channel, 1, bias=False)
        )

        self.conv=nn.Conv3d(2, 1, kernel_size=7, padding=7//2, bias=False)
    ...
    def forward(self, x):
        ...
        max_result, _ = torch.max(output_c, dim=1, keepdim=True) 
        avg_result = torch.mean(output_c, dim=1, keepdim=True)
        output = (max_result + avg_result) / 2
        output = self.sigmoid(output)
        return output_c * output
